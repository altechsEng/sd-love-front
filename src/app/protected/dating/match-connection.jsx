...
export default function MatchConnection({ navigation }) {
     ...
     const maleImage = profile?.match_user?.user_infos?.qP1 === "Homme" ? `${BaseImageUrl}/${profile?.match_user?.user_image}` : image;
     const femaleImage = profile?.match_user?.user_infos?.qP1 === "Femme" ? `${BaseImageUrl}/${profile?.match_user?.user_image}` : image;
     ...
     useEffect(() => {
          console.log(image, "user data....", profile?.score, profile)
          checkEngagement();
     }, [])

     const requestEgagement = async () => {
          let token = await AsyncStorage.getItem("user_token");
          let data = { matchId: profile.match_id }
               ...
               axios
                    ...
                    .then(async (res) => {
                         if (res.data.error || res.data.status == 500 || res.data.status === 401) {
                              ...
                              router.navigate("EngagementRequestSent", { profile })
                         }
                    })
          ...
          <View style={{ backgroundColor: "white", alignItems: "center", flex: 1 }}>
               ...
               <LinearGradient colors={["rgba(215, 168, 152, 1)", "rgba(255, 255, 255, 0.03)"]} start={{ x: 0, y: 0 }} end={{ x: 0, y: 1 }} style={{ height: SCREEN_HEIGHT * 0.6, width: "100%", position: "relative" }} >
                    <View style={{ paddingHorizontal: 20, backgroundColor: "" }}>
                         ...
                         <TouchableOpacity onPress={() => router.back()} style={{ alignItems: "flex-start", justifyContent: "flex-start" }}>
                    ...
                    <CustomSemiBoldPoppingText style={{ textAlign: "center", marginVertical: 20 }} color="#D0917B" fontSize={TEXT_SIZE.title + 8} value={`${profile?.score}%` || "89%"} />
                    ...
                    <View style={{ position: "relative", flexDirection: "row", alignItems: "center", justifyContent: "center" }}>
                         <View style={{ transform: [{ rotate: "-4.58deg" }], position: "absolute", top: hp(2), left: 40, zIndex: 8, overflow: "hidden", height: 240, width: 170, borderRadius: 20, padding: 3, backgroundColor: "white" }}>
                              <Image source={{ uri: `${maleImage}` }} style={{ borderRadius: 20, width: "100%", height: "100%" }} resizeMode='cover' />
                         </View>
                         ...
                         <View style={{ position: "absolute", top: hp(8), right: 40, transform: [{ rotate: "11deg" }], zIndex: 10, overflow: "hidden", height: 240, width: 170, borderRadius: 20, padding: 3, backgroundColor: "white" }}>
                              <Image source={{ uri: `${femaleImage}` }} style={{ borderRadius: 20, width: "100%", height: "100%" }} resizeMode='cover' />
                         </View>
                    </View>
               </LinearGradient>
               ...
               <View style={{ alignItems: "center", width: "85%", justifyContent: "center", marginTop: 50 }}>
                    ...
                    <CustomRegularPoppingText style={{ textAlign: "center", width: "80%", margin: "auto" }}
                         value={`You and ${profile?.match_user?.firstname} ${profile?.match_user?.lastname} have matched, start a conversation with each other now.`} color="#5E5E5E" fontSize={TEXT_SIZE.medium} />
               </View>
               ...
               {hasEngagementRequest
                    ...
                    <TouchableOpacity className='flex items-center ' onPress={() => requestEgagement()} style={{ marginTop: 50, width: "80%", paddingVertical: 10, borderRadius: 20, backgroundColor: COLORS.primary }}>
                         {buttonLoading ?
                              <AntDesign className='animate-spin' name="loading2" size={24} color="white" />
                              :
                              <CustomRegularPoppingText fontSize={TEXT_SIZE.small + 2} value={`Engage with ${profile?.match_user?.firstname}`} style={{ textAlign: "center" }} color={"white"} />
                         }
                    </TouchableOpacity>
               }

               <TouchableOpacity onPress={() => router.navigate("/protected/dating/match-connection", { profile })} style={{ marginTop: 10, width: "80%", paddingVertical: 10, borderRadius: 20, backgroundColor: "rgba(215, 168, 152, 0.13)" }}>
