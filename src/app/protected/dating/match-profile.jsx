import { CustomMeduimPoppingText, CustomRegularPoppingText, CustomSemiBoldPoppingText } from "../../../components/text";
import { HomeFeedComment, HomeFeedHeart, MatchProfileArrowBack, MatchProfileBirthDay, MatchProfileEducation, MatchProfileFaithChurch, MatchProfileFaithIcon1, MatchProfileFaithOccupation, MatchProfileHome, MatchProfileLang, MatchProfilePlus, MatchProfileSexIcon, MatchProfileSmallHeart, MatchProfleSmallFace, PostScreenBookMark, PostScreenDots } from "../../../components/vectors";
import { COLORS, TEXT_SIZE, FAMILLY, BaseImageUrl, POST_LIMIT, BasePostImageUrl } from "../../../utils/constants";
import React, { useEffect, useRef, useState } from "react"
import { View, Text, Image, TouchableOpacity, Dimensions, FlatList, Pressable, ScrollView } from "react-native"
			...
			axios
				...
				.then(async (res) => {
					if (res.data.error || res.data.status == 500 || res.data.status === 401) {
						...
						router.navigate("EngagementRequestSent", { item })
					}
				})
			...
			<View key={item?.key} style={{ flex: 1, marginVertical: 20, marginRight: 20, flexDirection: "column" }}>
				<View style={{ flexDirection: "row", justifyContent: "space-evenly", alignItems: "center" }}>
					<View clasName='' style={{ flex: 2, flexDirection: "row" }}>
						<TouchableOpacity style={{ borderRadius: 50, height: 40, width: 40, overflow: "hidden", alignItems: "center" }}><Image source={item?.user?.user_image ? { uri: `${BaseImageUrl}/${item?.user?.user_image}` } : null} resizeMode="cover" style={{ height: "100%", width: "100%" }} /></TouchableOpacity>
				...
				<Pressable onPress={() => router.navigate("Post", { item })} style={{ margin: 0, padding: 0, overflow: "hidden" }}>
	...
	return (
		<ScrollView showsHorizontalScrollIndicator={false} showsVerticalScrollIndicator={false} style={{ backgroundColor: "white", padding: 20, flex: 1 }}>
			...
			<View style={{ height: SCREEN_HEIGHT * 0.5, borderRadius: 20, alignItems: "center", position: "relative", overflow: "hidden" }}>
				<TouchableOpacity onPress={() => router.goBack()} style={{ top: 15, position: "absolute", left: 15, zIndex: 10 }}>
					...
					<View>
						<View style={{ flexDirection: "row", alignItems: "center", marginBottom: 20 }}>
							...
							<View>
								...
								<CustomRegularPoppingText style={{ lineHeight: 15 }} color={null} fontSize={TEXT_SIZE.primary} value={item?.match_user?.user_infos?.qP17 || "Not specified"} />
							</View>
						</View>
					</View>
			...
			{activeSubCat == "Posts" && (
				<View>
					<FlatList
						data={allMatchPost.length > 0 ? allMatchPost : null}
			...
			{activeSubCat == "Photos" && (
				<View style={{ marginTop: 20, flexDirection: "row", flexWrap: "wrap" }}>
					<FlatList data={allMatchImages.length > 0 ? allMatchImages : null}
