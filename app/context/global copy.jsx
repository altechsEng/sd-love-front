...
export const GlobalVariableProvider = ({
	...
	const loadData = async () => {
		try {
			...
			if (result) {
				...
				setUserData({ ...data?.user, userInfo: data.user_info, dob: data?.user_info?.qP2, loveLang: data?.user_info?.qP13, interests: data?.user_info?.qP16, importantFactor: data?.user_info?.qP15 })
	...
	useEffect(() => {
		...
		// This will ensure that the user data is available globally
		// and can be accessed by any component that uses the context
		loadData()
	}, [])
	...
	return (
		<GlobalVariableContext.Provider
			value={{
				...
				refreshUserData, setRefreshUserData, loadData, checkLogin
			}}
		>
			{children}
		</GlobalVariableContext.Provider>
